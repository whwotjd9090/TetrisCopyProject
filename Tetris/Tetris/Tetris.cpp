...
void Tetris::InitIntro()
{
	isPressed_UP = false;
	isPressed_DOWN = false;
	isPressed_RETURN = false;

	high_score_file_read.open("highscore.dat");

	if(high_score_file_read)
	{
	}
...
void Tetris::DrawHelp() const
{
	...
	std::cout << "\tTetris\t네 줄 클리어800 X 현재 레벨" << std::endl << std::endl;
...
void Tetris::InitGame()
{
	...
	combo_count = 0;
	remain_line = 5;
	block_down_frame = FRAME_RATE_PLAY;
	lock_delay_frame = 0.5 * FRAME_RATE_PLAY;
	...
	*/
	...
	// 0, 1, 2		: 왼쪽 벽 3개
	// 13, 14, 15	: 오른쪽 벽 3개
	for(size_t i = 0; i < WALL_LEFT_BOTTOM; i++)
	...
	// 24, 25, 26 : 아래쪽 벽 3개
	for(size_t i = 0; i < STAGE_FIELD_WIDTH; i++)
...
void Tetris::Play()
{
	...
	SoundManager::GetInstance()->SoundStop(SOUND_PLAY_BACKGROUND);
	ShowGameOverEffect();
	DrawHighScore();
	SoundManager::GetInstance()->SoundStop(SOUND_GAMEOVER);
}
void Tetris::ShowGameOverEffect()
{
	for(size_t i = BLOCK_MOVEABLE_TOP; i <= BLOCK_MOVEABLE_BOTTOM; i++)
	{
		for(size_t j = BLOCK_MOVEABLE_LEFT; j <= BLOCK_MOVEABLE_RIGHT; j++)
		{
			if(game_field.at(i).at(j) == 1)
			{
				auto draw_pos = PositionToDrawPosition({j, i});

				WindowConsoleControl::SETTEXTCOLOR(WINDOW_CONSOLE_COLOR::GRAY, static_cast<WINDOW_CONSOLE_COLOR>(CONSOLE_COLOR_BACKGROUND));
				WindowConsoleControl::SETPOSITION(draw_pos.first, draw_pos.second);
				std::cout << "■";
			}
	...
	std::uniform_int_distribution<COLOR16> go_color(0U, 15U);
	isPressed_RETURN = false;

	SoundManager::GetInstance()->SoundPlay(SOUND_GAMEOVER);

	auto go_start = gClock::now();
	gFrame go_counter;
	...
	while(true)
	{
		...
		auto go_now = gClock::now();
		go_counter = std::chrono::duration_cast<gFrame>(go_now - go_start);

		if(go_counter.count() >= FRMAE_RATE_GAMEOVER)
		{
			go_start = go_now;
			WindowConsoleControl::SETTEXTCOLOR(static_cast<WINDOW_CONSOLE_COLOR>(go_color(dre)), static_cast<WINDOW_CONSOLE_COLOR>(CONSOLE_COLOR_BACKGROUND));
			WindowConsoleControl::SETPOSITION(0, 6);
